% Simulation of the advection-diffusion of Ricca factor in wheet seeedling 
% This code was used to obtain the plot in Fig.4c
...
kx_ves = 7.8e-4; % Xylem vessel conductivity [mol m-1 Pa-1 s-1]
l_ves = 1e-5; % Xylem vessel diamater [m]
mu = 1.8*1e-5; % Molar volume of water [m3 mol-1]
L = 0.2; % Leaf-to-leaf length [m]
T = 150; % Comsol simulation time [s]
%% Load Comsol data of gradient of xylem pressure 
NTP = load('File_F7_Xylem_Pressure_Gradient_Wheat_E0_0.mat'); %Non-transpiring plant
grad_x_NT = NTP.der_x;

TP = load('File_F8_Xylem_Pressure_Gradient_Wheat_E0_1.2.mat'); %Transpiring plant
grad_x_T = TP.der_x;
...
dx  = L/x_length; % Spatial increments

t = 0:dt:T;
x = 0:dx:L;

%% Run Lagrangian simulation 
Np = 50; % number of elicitors particles;

%Line injection of Np elicitors at wound site, x = -L/2, y =[0 l_ves]
% Transpiring plant
xpart_T(1:Np) = 0;
